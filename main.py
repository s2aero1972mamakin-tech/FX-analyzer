...
def _batch_find_index_asof(df: pd.DataFrame, asof_date: date) -> int:
    """dfのindex（DatetimeIndex）から、asof_date(日本日付)までの最終行index位置を返す。見つからなければ-1。"""
    if df is None or len(df) == 0:
        return -1
    try:
        idx_dates = pd.to_datetime(df.index).date
        mask = idx_dates <= asof_date
        if not mask.any():
            return -1
        ...
        try:
            for i in range(len(df) - 1, -1, -1):
                try:
                    if pd.to_datetime(df.index[i]).date() <= asof_date:
                        return i
                except Exception:
                    continue
        except Exception:
...
def _batch_simulate_order_weekly(
    df_full: pd.DataFrame,
    asof_date: date,
    order: dict,
    max_hold_weeks: int = 4,
    max_entry_wait_weeks: int = 2,
) -> dict:
    """週足OHLCで簡易シミュレーション（紙トレ）。
    - LIMIT/STOPの約定を簡易判定（最大max_entry_wait_weeks週まで待つ）
    - 約定後、SL/TPヒットを週足High/Lowで判定
    - 同一週でSLとTPが同時に触れた場合は保守的にSL優先（最悪側）
    """
    res = {"filled": False, "fill_date": "", "exit_type": "NO_FILL", "exit_date": "", "final_r": 0.0, "mfe_r": 0.0, "mae_r": 0.0, "hold_weeks": 0}
    if df_full is None or len(df_full) < 5 or not isinstance(order, dict):
        return res

    side = str(order.get("side") or "LONG").upper()
    kind = str(order.get("kind") or order.get("entry_kind") or "").upper()
    try:
    ...
    i0 = _batch_find_index_asof(df_full, asof_date)
    if i0 < 0:
        return res

    fill_i = None
    for j in range(1, max(1, int(max_entry_wait_weeks)) + 1):
        i = i0 + j
        if i >= len(df_full):
            break
        row = df_full.iloc[i]
        try:
            hi = float(row.get("High", float("nan")))
    ...
    res["filled"] = True
    try:
        res["fill_date"] = str(pd.to_datetime(df_full.index[fill_i]).date())
    except Exception:
        res["fill_date"] = ""
    ...
    exit_i = None
    exit_price = None
    exit_type = "WEEK_CLOSE"

    for k in range(0, max(1, int(max_hold_weeks))):
        i = fill_i + k
        if i >= len(df_full):
            break
        row = df_full.iloc[i]
        ...
        if side == "LONG":
            mfe = max(mfe, (hi - entry) / r_denom)
            mae = min(mae, (lo - entry) / r_denom)
            sl_hit = lo <= sl
            tp_hit = hi >= tp
        else:
            mfe = max(mfe, (entry - lo) / r_denom)
            mae = min(mae, (entry - hi) / r_denom)
            sl_hit = hi >= sl
            tp_hit = lo <= tp
    ...
    if exit_i is None:
        exit_i = min(fill_i + max(0, int(max_hold_weeks) - 1), len(df_full) - 1)
        try:
            exit_price = float(df_full.iloc[exit_i].get("Close", entry))
        except Exception:
            exit_price = entry
        exit_type = "WEEK_CLOSE"

    final_r = ((float(exit_price) - entry) / r_denom) if side == "LONG" else ((entry - float(exit_price)) / r_denom)

    res["exit_type"] = exit_type
    try:
        res["exit_date"] = str(pd.to_datetime(df_full.index[exit_i]).date())
    except Exception:
        res["exit_date"] = ""
    res["final_r"] = float(final_r)
    res["mfe_r"] = float(mfe)
    res["mae_r"] = float(mae)
    res["hold_weeks"] = int(exit_i - fill_i + 1) if exit_i is not None else 0
    return res
